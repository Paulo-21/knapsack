...
// Algorithme Branch and Bound pour le sac à dos 0/1
int sac_a_dos_branch_and_bound(int C, vector<Objet>& objets) {
    sort(objets.begin(), objets.end(), comparer); // Trie les objets selon le ratio valeur/poids décroissant
    ...
    while (!file.empty()) {
        v = file.front(); // On récupère un nœud à traiter
        file.pop();
